...
**Connecting a source imports its history, not the last 90 days.** The first sync of a source takes everything the API will give (years, not a window); every sync after it resumes from the last day you already have, so nothing is re-fetched and nothing is left behind. Where an API has a hard ceiling the row says so instead of looking broken - Spotify only ever returns your last 50 plays, so a sync covers a few days and your full listening history comes from their account export.
