...
**Email** (Settings → Channels → Email) gives agentqs a way to mail you: plain SMTP (send-only - Gmail app passwords, Fastmail, SES, Postmark), or your Google account through its own send-only grant that reuses the Google key you already saved, with read access added only if you tick *Capture replies*. Prove it with the Test button or `agentqs mail test --to you@example.com`. The same transport powers **Forgot password?** on the sign-in page: a single-use link, good for 30 minutes, that also signs out every existing session. No email set up? `agentqs password --set` on the machine resets it at a hidden prompt.
