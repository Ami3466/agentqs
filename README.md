...
**Polling, not just push.** Name a conversation under Settings → Channels → *Also poll this channel* (a channel name, a comma-separated list, or `*` for every conversation the bot is in, DMs included — because "my week of logs vanished" almost always means they were written somewhere other than the one channel the poll was aimed at) and agentqs checks it on its own schedule (hourly by default) as well as receiving webhooks - so the messages you sent while the webhook was down are still collected on the next sweep instead of being lost. Both paths key the capture on the platform's own message identity (Slack's channel + timestamp, Telegram's chat + message id), so a message that arrives twice lands once, and the bot's own replies are never captured as entries. The two directions are also told apart on the row: a poll that could not reach Slack says exactly that, instead of accusing Slack of sending a message the app refused, and a poll quietly collecting everything never certifies a webhook that has not fired once - that combination gets its own warning pointing at the app's Event Subscriptions. It is an ordinary due-source swept by the in-app scheduler **on the same host as your record**: no external CI minutes to run out, nothing to keep in sync with a second repo. `agentqs sync --id slack` runs it by hand. **Email is the fourth channel**, and the only one with no webhook: every email agentqs sends you ends with a short `aqs#` footer, your reply quotes it, and one Gmail search on that tag finds every reply to every notification - no labels, no inbound host, no tunnel. Replies are read only on the Gmail transport with *Capture replies* ticked (SMTP is send-only, by design); a reply counts only if it carries your instance's token, comes from an address agentqs has mailed and is not in Spam, and it lands under Gmail's own message id with the quoted original cut off (a heuristic: good for Gmail, Apple Mail and Outlook, not exact for every client). `agentqs sync --id email` runs it by hand; `npm run email:test` proves it.

**Agent rules** (Settings → Agent) message you the other way: *when X, ping me*. X is a time (an 8pm evening brief) or a data line crossing (resting HR over 55, social over 60 min today) - the threshold is a plain compare over your record, no AI. The message is a fixed line, or an AI brief the agent writes from your day. The in-app scheduler checks every 15 minutes, so an alert is as fresh as the source behind its metric. Set them in the app, from the CLI (`agentqs rules add`), or let the agent add its own via MCP. A plain daily **notification** (Settings → Channels, `agentqs notifications add`, or the `notification_upsert` MCP tool) is the same send with no trigger to think about: a fixed line like an 8pm "How was your day?", or `--recap`, a recap written from your record when it sends - to Slack, Telegram or your email.

**Email** (Settings → Channels → Email) gives agentqs a way to mail you: plain SMTP (send-only - Gmail app passwords, Fastmail, SES, Postmark), or your Google account through its own send-only grant that reuses the Google key you already saved, with read access added only if you tick *Capture replies*. For SMTP, port 587 is STARTTLS and 465 is TLS from the first byte; a server that offers neither is refused by design, not downgraded, because your password would cross in the clear - and for Gmail the password is an **app password** (2-Step Verification has to be on to create one), not your account password. Prove it with the Test button, `agentqs mail test --to you@example.com`, or `npm run mail:live`, which sends through your own server and reads every message back over IMAP (it skips cleanly when no credentials are set). Setup, replies, notifications and reset in full: [docs/email-and-notifications.md](docs/email-and-notifications.md). The same transport powers **Forgot password?** on the sign-in page: a single-use link, good for 30 minutes, that also signs out every existing session. No email set up? `agentqs password --set` on the machine resets it at a hidden prompt.
...
**Every connection shows its receipts, and clicking one opens its data.** A connected row carries a Connected badge, what actually landed (days, events, date range) and when it last synced - all counted from the record, so a connected-but-empty source reads "no data yet" instead of passing for a healthy one. Whoop rows also name *which* athlete they are authorized as, so two straps are never twins. Click the row and the Journal opens filtered to that source alone: "what did this give me?" is one click, never a guess.
...
**A source can start over without you signing in again.** `agentqs source reset <id>` throws away everything a source landed - its daily file, its events, Whoop's per-minute heart rate - and keeps the connection: the key, the login, the schedule. The next sync then re-walks the source's whole history into a clean file. This is how you undo a bad import, and it is the only way: a sync *merges*, so it can correct a value it writes again but it can never remove a day it now knows better than to write at all. Re-syncing after a fix leaves the bad days exactly where they were; resetting first is what clears them. Removing the source instead (`source remove`) also takes the key, which is why cleaning up used to mean signing back in to everything.
...
- **Derived indexes - rebuildable, never committed.** A SQLite cache for SQL, a full-text index, on-device embeddings for semantic search, and a photo index. `agentqs rebuild` recreates all of them from the record, byte-identical. Nothing you click can trigger that rebuild: it re-reads the whole record, SQLite is synchronous, and one of those on a web request is not a slow page - it is every page in the app hanging at once. Every action instead patches only the rows it changed, and the app refuses (loudly, naming `agentqs rebuild`) rather than re-deriving your record behind a button.
...
- **Raw files on request, from Drive.** For raw content the record deliberately never stores (emails, whole message threads, PDFs), keep it in a Google Drive folder and point agentqs at it in Settings → Data → Drive import. `agentqs drive list` shows what's there and `agentqs drive pull <file>` reads one file's text on demand (a PDF comes back as its extracted text, not a stub) - nothing is synced into your record, and the chat agent pulls a file only when a question needs it. Read-only access, a separate grant from the backup one.
- **Token use is explicit.** Capture and search are local. AI runs only when you chat, structure prose, or run a channel - CLI-agent workflows skip it entirely. The local models those searches use (the text embedder, photo search and captioning, built-in Whisper) load on demand and are unloaded again after five minutes idle, so a single search or captioned photo doesn't leave hundreds of MB parked for the life of the process - `AGENTQS_MODEL_IDLE_MS` tunes the window.
...
- **No action can hang the app.** Structuring, keeping and discarding all cost work proportional to what you changed, not to the size of your record, and they run as background jobs on one serial queue - so a slow merge answers with a job to follow instead of holding the connection, and closing the tab no longer kills it. `npm run freeze:test` proves it against a seeded 200,000-event record: structuring an item takes about 50ms where re-deriving that record takes seconds.

## License

[MIT with the Commons Clause](LICENSE) - free to use, modify and self-host, including inside a company; not for resale.
