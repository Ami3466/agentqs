...
async function main(): Promise<void> {
  const journalFile = firstExisting([
    ...
    "~/Desktop/mentor-journal/data/notion-pages/2022-summarize-table.csv",
    "~/Library/CloudStorage/GoogleDrive-amit@flowengine.cloud/My Drive/Journal Notion.csv",
  ]);
  const textFile = firstExisting([
    "~/Desktop/mentor-journal/data/journal/journal_texts.csv",
    "~/Desktop/agentqs-mentor-data/record/daily/journal_texts.csv",
  ]);
  ...
  const pages = importNotionPages(expandHome("~/Desktop/mentor-journal/data/notion-pages"), rDir);
