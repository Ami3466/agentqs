  ...
  return (
    <form onSubmit={save} className="md:grid md:grid-cols-[190px_1fr] md:items-start md:gap-8 lg:grid-cols-[210px_1fr] lg:gap-10">
      ...
      <ChannelCard
        ...
        providers={keyedProviders.map((p) => ({ id: p.id, label: p.label || providerTypeOf(p.type)?.label || p.type }))}
      />
      <NudgesPanel tzResolved={config.timezoneResolved} />
      </>
      ) : null}
...
/**
 * Daily nudges — scheduled OUTBOUND messages the app sends you (e.g. an 8pm "how
 * was your day?"). The list is derived from GET /api/nudges so it survives reload;
 * the flash line is a confirmation only. Refetches are quiet (no unmount).
 */
function NudgesPanel({ tzResolved }: { tzResolved: string }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (res.ok) setRows(((await res.json()).nudges as NudgeRow[]) ?? []);
    } catch {
