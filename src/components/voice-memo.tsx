...
export function VoiceMemo() {
  ...
  async function upload(mime: string) {
    ...
    try {
      ...
      form.append("audio", blob, `memo${extForMime(mime)}`);
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open ? (
        <div className="absolute right-0 z-50 mt-2 w-[300px] max-w-[calc(100vw-2rem)] rounded-xl border border-border bg-card p-4 shadow-xl">
          <Panel
            cap={cap}
            phase={phase}
            elapsed={elapsed}
    ...
    return (
      <div>
        ...
        <p className="mt-2 text-[12px] text-muted-fg">
          Set <code className="font-mono text-fg">WHISPER_BIN</code> to a local Whisper wrapper for
          private, no-cost transcription, or add an OpenAI key to use hosted Whisper.
        </p>
        <button
          type="button"
          onClick={onClose}
      ...
      <div>
        ...
        <div className="rounded-lg border border-accent/40 bg-accent/10 p-3">
          <div className="mb-1 flex items-center gap-1.5 text-[11px] font-medium text-accent">
            <Check width={12} height={12} /> memo saved to inbox · {result.pending} pending
          </div>
