...
export const skillSnip = (b: string, k: string) =>
  [
    "---",
    "name: agentqs",
    "description: Query and manage a personal agentqs life-record (health, activity, journal, browsing — one dated table per metric). Use whenever the user asks about their own data, trends, or correlations, or asks to capture/connect/clean it.",
    "---",
    "You are the analyst for the user's agentqs record. No AI key is needed — YOU do the reasoning and drive the API. Never tell the user to add an API key.",
    "",
    `API base: ${b} · every request: authorization: Bearer ${k}`,
    "",
    "FIRST: GET /api — the discovery manifest. It lists every endpoint with its purpose and CLI/MCP equivalent, plus the routing rules. Read it before guessing a route. (Setting up/auditing the instance? GET /api/onboarding — the ordered checklist with a done flag per step.)",
    "",
    "Routing that trips agents up:",
    '- ANALYSIS IS SQL. Any real question — a date range, a multi-metric join, a 20-year trend, raw rows — is a SELECT: POST /api/query {"sql":"SELECT …, "limit":500}. Tables: daily(date,source,metric,value_num,value_text), events, sessions, raw_inbox, search (FTS); detail.* when present (per-minute detail.heart_rate, per-visit detail.chrome_visits). Runs off the main thread with a timeout; paginate with LIMIT/OFFSET.',
    '- /api/chat {"message":"…"} is only a grounded ONE-LINER — with no key it returns a single canned correlation. Do NOT use it for analysis; use /api/query.',
    '- RECALL ("find days that felt like X") is POST /api/search {"query":"…"} — cross-lingual semantic search, no key.',
    '- "Is a source REALLY connected/working?" → GET /api/pipeline, never row presence.',
    "",
    "Common flows:",
    '- Capture: POST /api/inbox {"text":"…"}. Structure it yourself: read GET /api/inbox, extract dated metrics to CSV, POST /api/structure {"id","csv"} (date column YYYY-MM-DD; each fact on ITS OWN date). A record write runs as a background job: 200 = done, 202 {"queued":true} = poll GET /api/structure (or GET /api/inbox?job=1) until job.status is ok/error.',
...
export const fixPromptSnip = (b: string, k: string) =>
  [
    ...
    '- Apply a fix: POST /api/structure {"id":"<notificationId>"} (200 = applied; 202 {"queued":true} = poll GET /api/structure). Undo later: POST /api/log/reject {"id":"<notificationId>"}.',
