/**
 * The smart-input contract for the Chat box. One line the user types is routed by
 * its prefix into one of three modes:
 *
 *   plain text  → talk to the skill (the model / grounded answer)
 *   `//`        → a memo: appended raw to the inbox, no LLM, no reply
 *   `/`         → a command: /sync · /structure · /new · /skill
 *
 * `//` is checked BEFORE `/` so a memo never reads as a command. Pure data + string
 * helpers only (no fs, no React) so the exact same routing is shared by the client
 * input box and server-side tests — the dispatch can't drift.
 */
...
export const COMMANDS: Command[] = [
  { cmd: "/sync", desc: "Pull the latest from connected sources" },
  { cmd: "/structure", desc: "Turn pending inbox items into data" },
  { cmd: "/new", desc: "Start a fresh session" },
  { cmd: "/skill", desc: "Switch skill — /skill mentor · therapist · coach" },
];
